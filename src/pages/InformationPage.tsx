...
export default function InformationPage({ page }: { page: Page }) {
  ...
  const contact = <p><a className="font-semibold text-olive-800 underline" href={`mailto:${email}`}>{email}</a> · <a className="font-semibold text-olive-800 underline" href="tel:+201063167656">{phone}</a></p>
  ...
  return <><Navbar /><div className="policy-page min-h-screen pb-20 pt-24 sm:pt-28"><div className="mx-auto max-w-6xl px-4 sm:px-6">
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start">
      ...
      {page === 'about' && <>
        ...
        <Section title={ar ? 'ما نقدمه' : 'What we offer'}><p>{ar ? 'نقدم مجموعة متنامية من الكورسات الرقمية العملية المصممة لمساعدتك على اكتساب مهارات قابلة للتطبيق وبناء مصادر دخل عبر الإنترنت. يتم تحديث قائمة الكورسات ومحتواها وأسعارها باستمرار، وستجد دائمًا التفاصيل الحالية قبل إتمام الدفع.' : 'We offer a growing collection of practical digital courses designed to help you develop useful skills and build income opportunities online. Our course catalog, content, and prices are updated regularly, and you will always see the current details before completing payment.'}</p><Link to="/#courses" className="font-bold text-olive-800 underline">{ar ? 'استكشف الكورسات المتاحة' : 'Explore available courses'}</Link></Section>
      </>}
      {page === 'contact' && <Section title={ar ? 'بيانات التواصل' : 'Contact details'}><p>{ar ? 'يسعدنا مساعدتك في الأسئلة المتعلقة بالكورسات أو الدفع أو الوصول إلى المحتوى.' : 'We are happy to help with questions about courses, payment, or access to your content.'}</p><div className="mt-5 grid gap-3"><a href={`mailto:${email}`} className="flex items-center gap-3 rounded-xl bg-olive-50 p-4"><Mail className="h-5 w-5" /><span><strong>{ar ? 'البريد الإلكتروني:' : 'Email:'}</strong> {email}</span></a><a href="tel:+201063167656" className="flex items-center gap-3 rounded-xl bg-olive-50 p-4"><Phone className="h-5 w-5" /><span><strong>{ar ? 'الهاتف:' : 'Phone:'}</strong> {phone}</span></a><div className="flex items-center gap-3 rounded-xl bg-olive-50 p-4"><MapPin className="h-5 w-5" /><span><strong>{ar ? 'عنوان النشاط:' : 'Business address:'}</strong> {ar ? 'العصافرة، الإسكندرية، مصر' : 'Asafra, Alexandria, Egypt'}</span></div></div></Section>}
      {page === 'privacy' && <>
